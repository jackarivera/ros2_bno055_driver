  ...
  connection_rate_(4.0)
{
  ...
  declare_parameter("angular_velocity_stdev", 0.0);
  declare_parameter("linear_acceleration_stdev", 0.0);
  declare_parameter("magnetic_field_stdev", 0.0);
  declare_parameter("orientation_stdev", 0.0);

  declare_parameter("calibration.accelerometer_offset", std::vector<int64_t>({0, 0, 0}));
  declare_parameter("calibration.gyroscope_offset", std::vector<int64_t>({0, 0, 0}));
  declare_parameter("calibration.magnetometer_offset", std::vector<int64_t>({0, 0, 0}));
  declare_parameter("calibration.accelerometer_radius", 0);
  declare_parameter("calibration.magnetometer_radius", 0);
...
BNO055Driver::on_activate(const rclcpp_lifecycle::State &)
{
  ...
  port_->write_command_.address = BNO055Register::OPR_MODE;
  port_->write_command_.length = 1;
  auto operation_mode = BNO055OperationMode::IMU;
